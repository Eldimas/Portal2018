...
export class ChecklistDatabase {
    ...
    // tslint:disable-next-line:typedef
    updateFlatItem(node: TodoItemNode, title: string) {
        // node.item = name;
        node.title = title;
...
@Component({
    selector: 'app-edit-menu2',
    templateUrl: './edit-menu2.component.html',
    styleUrls: ['./edit-menu2.component.scss'],
    providers: [ChecklistDatabase]
})
export class EditMenu2Component {
    ...
    editItem(node: TodoItemFlatNode) {
        ...
        const navig$ = this._navigService.getNavigById(node.id);

        navig$.subscribe(res => {
            const navig = res;

            const dialogRef = this.editDialog.open(DialogEditItemComponent, {
                data: res,
                disableClose: true
            });

            dialogRef.afterClosed().subscribe(result => {
                if (result !== false && result !== 'delete') {
                    const navigUpdate = new Navig(result);
                    console.log('navigUpdate: ', navigUpdate);

                    this._navigService.updateNavig(navig).subscribe(resNav => {
                        console.log(resNav);
    ...
    saveNode(
        ...
        this._navigService.addNavig(navig).subscribe(res => {
            console.log(res);
